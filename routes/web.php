...
Route::get('/sitemap.xml', function () {
    ...
    $publishedProjects = \App\Models\Project::query()
        ->whereIn('status', ['published', 'active', 'approved', 'investor_visible'])
        ->latest()
        ->limit(1000)
        ->get();

    foreach ($publishedProjects as $project) {
        $urls->push(route('frontend.projects.show', $project));
    }
...
Route::middleware(['web', 'frontend.locale'])->group(function () {
    ...
    Route::middleware(['auth:web', 'frontend.verified.policy'])->group(function () {
        ...
        Route::prefix('security')->name('security.')->group(function () {
            ...
            Route::post('/recovery-codes/regenerate', [SecurityController::class, 'regenerateRecoveryCodes'])->name('recovery-codes.regenerate');
        });

        Route::post('/projects/{project}/request-funding', [FrontendProjectController::class, 'requestFunding'])->name('frontend.projects.requestFunding');
        ...
        Route::post('/projects/{project}/invest', [FrontendProjectController::class, 'invest'])->name('frontend.projects.invest');
        Route::delete('/projects/{project}/interest', [FrontendProjectController::class, 'removeInterest'])->name('frontend.projects.interest.remove');

        Route::get('/investor/investments', [InvestorDashboardController::class, 'investments'])->name('investor.investments');

        Route::post('/student/requests/{requestItem}/respond', [AcademicDashboardController::class, 'respondToRequest'])->name('student.requests.respond');

        Route::get('/settings/academic', [AcademicDashboardController::class, 'settings'])->name('settings.academic');
        Route::post('/settings/academic/update', [AcademicDashboardController::class, 'updateSettings'])->name('settings.academic.update');

        Route::get('/settings/investor', [InvestorDashboardController::class, 'settings'])->name('settings.investor');
        Route::post('/settings/investor/update', [InvestorDashboardController::class, 'updateSettings'])->name('settings.investor.update');

        Route::prefix('investor/projects')->name('investor.projects.')->group(function () {
            Route::get('{project}/summary', [InvestorProjectSummaryController::class, 'show'])->name('summary');
            Route::get('{project}/pitch-deck/download', [InvestorProjectSummaryController::class, 'download'])->name('pitch-deck.download');
        });
        ...
        Route::get('/notifications/latest', [FrontNotificationController::class, 'latest'])->name('frontend.notifications.latest');
