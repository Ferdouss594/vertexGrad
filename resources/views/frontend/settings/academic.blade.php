...
<div class="min-h-screen pt-20 sm:pt-24 lg:pt-28 pb-10 sm:pb-12 bg-theme-bg transition-colors duration-300 overflow-x-hidden">
    <div class="w-full max-w-7xl mx-auto px-3 sm:px-6 lg:px-8">
        ...
        <form action="{{ route('settings.academic.update') }}" method="POST" enctype="multipart/form-data" class="space-y-5 sm:space-y-6 lg:space-y-8">
            @csrf
                    ...
                    <div class="flex-1 min-w-0">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-5">
                            <div class="theme-panel-soft rounded-2xl p-4 min-w-0">
                                ...
                                <input type="text" name="full_name" value="{{ old('full_name', $user->name) }}" class="w-full min-w-0 p-3 rounded-xl border border-theme-border bg-theme-surface text-theme-text text-sm sm:text-base">
                                @error('full_name') <p class="text-sm text-red-500 mt-2">{{ $message }}</p> @enderror
                            </div>
                            ...
                            <div class="theme-panel-soft rounded-2xl p-4 min-w-0">
                                ...
                                <input type="text" name="academic_title" value="{{ old('academic_title', $user->academic_title ?? '') }}" class="w-full min-w-0 p-3 rounded-xl border border-theme-border bg-theme-surface text-theme-text text-sm sm:text-base">
                                @error('academic_title') <p class="text-sm text-red-500 mt-2">{{ $message }}</p> @enderror
                            </div>
                            ...
                            <div class="theme-panel-soft rounded-2xl p-4 min-w-0">
                                ...
                                <input type="email" name="email" value="{{ old('email', $user->email) }}" class="w-full min-w-0 p-3 rounded-xl border border-theme-border bg-theme-surface text-theme-text text-sm sm:text-base">
                            ...
                            <div class="theme-panel-soft rounded-2xl p-4 min-w-0">
                                ...
                                <input type="tel" name="phone" value="{{ old('phone', $user->phone ?? '') }}" class="w-full min-w-0 p-3 rounded-xl border border-theme-border bg-theme-surface text-theme-text text-sm sm:text-base">
                                @error('phone') <p class="text-sm text-red-500 mt-2">{{ $message }}</p> @enderror
                            </div>
                        </div>
                    </div>
                </div>
            ...
            <section class="theme-panel rounded-3xl sm:rounded-[2rem] lg:rounded-[2.5rem] p-4 sm:p-6 md:p-8">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-5">
                    <div class="theme-panel-soft rounded-2xl p-4 min-w-0">
                        ...
                        <input type="text" name="institution" value="{{ old('institution', $user->institution ?? '') }}" class="w-full min-w-0 p-3 rounded-xl border border-theme-border bg-theme-surface text-theme-text text-sm sm:text-base">
                        @error('institution') <p class="text-sm text-red-500 mt-2">{{ $message }}</p> @enderror
                    </div>
                    ...
                    <div class="theme-panel-soft rounded-2xl p-4 min-w-0">
                        ...
                        <input type="text" name="department" value="{{ old('department', $user->department ?? '') }}" class="w-full min-w-0 p-3 rounded-xl border border-theme-border bg-theme-surface text-theme-text text-sm sm:text-base">
                        @error('department') <p class="text-sm text-red-500 mt-2">{{ $message }}</p> @enderror
                    </div>
                </div>
            </section>
            ...
            <section class="theme-panel rounded-3xl sm:rounded-[2rem] lg:rounded-[2.5rem] p-4 sm:p-6 md:p-8">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-5">
                    <label class="theme-panel-soft rounded-2xl p-4 sm:p-5 flex items-start gap-3 sm:gap-4 cursor-pointer min-w-0">
                        <input type="checkbox" name="notif_status_change" value="1" class="mt-1 shrink-0" {{ old('notif_status_change', $user->notif_status_change ?? false) ? 'checked' : '' }}>
                        <div class="min-w-0">
                            <div class="text-theme-text font-bold break-words">{{ __('frontend.academic.notif_status') }}</div>
                            <div class="text-sm text-theme-muted mt-1 leading-6">{{ __('frontend.academic.notif_status_hint') }}</div>
                        </div>
                    </label>

                    <label class="theme-panel-soft rounded-2xl p-4 sm:p-5 flex items-start gap-3 sm:gap-4 cursor-pointer min-w-0">
                        <input type="checkbox" name="notif_investor_interest" value="1" class="mt-1 shrink-0" {{ old('notif_investor_interest', $user->notif_investor_interest ?? false) ? 'checked' : '' }}>
                        <div class="min-w-0">
                            <div class="text-theme-text font-bold break-words">{{ __('frontend.academic.notif_investor') }}</div>
                            <div class="text-sm text-theme-muted mt-1 leading-6">{{ __('frontend.academic.notif_investor_hint') }}</div>
                        </div>
                    </label>
                </div>
            </section>
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    statNumbers.forEach((el, index) => {
        ...
        el.textContent = '0';
        setTimeout(() => animateCount(el, value, 1000), 220 + (index * 120));
    });
