...
<div class="min-h-screen flex items-center justify-center py-12" style="background-color: {{ $darkBg }};">
    <div class="w-full max-w-lg p-10 rounded-2xl border border-primary/30 shadow-[0_0_50px_rgba(30,227,247,0.2)]" style="background-color: {{ $cardBg }};">
        ...
        <h2 class="text-3xl font-bold text-center text-light mb-6">Investor <span class="text-primary">Registration</span></h2>

        <form method="POST" action="{{ route('register.investor.post') }}" class="space-y-4">
            @csrf
            <div>
                <label class="block text-sm font-medium text-light/80 mb-1">Full Name / Entity Name</label>
            ...
            <div>
                <label class="block text-sm font-medium text-light/80 mb-1">Business Email</label>
            ...
            <button type="submit" class="w-full {{ $btnPrimaryClass }} py-3 mt-4">Create Investor Account</button>
        </form>
    </div>
</div>
@endsection